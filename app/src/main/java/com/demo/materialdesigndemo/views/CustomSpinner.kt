package com.demo.materialdesigndemo.views
 ...
 */
open class CustomSpinner @JvmOverloads constructor(
    context: Context,
        ...
        set(value) {
            ...
            adapter?.apply {
                if (value in 0 until count) {
                    ...
                    editText.setText("")
                    onItemSelectedListener?.onNothingSelected(this@CustomSpinner)
                }
            }
        }
    /**
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.CustomSpinner).run {
            getInt(R.styleable.CustomSpinner_android_gravity, -1).let {
                if (it > -1) {
                    gravity = it
                    editText.gravity = it
                } else {
            ...
            editText.isEnabled =
                getBoolean(R.styleable.CustomSpinner_android_enabled, editText.isEnabled)
            editText.isFocusable =
                getBoolean(R.styleable.CustomSpinner_android_focusable, editText.isFocusable)
            editText.isFocusableInTouchMode = getBoolean(
                R.styleable.CustomSpinner_android_focusableInTouchMode,
                editText.isFocusableInTouchMode
            )
            getColorStateList(R.styleable.CustomSpinner_android_textColor)?.let {
            ...
            getText(R.styleable.CustomSpinner_android_text)?.let {
            ...
            popup = when (getInt(R.styleable.CustomSpinner_spinnerMode, mode)) {
                MODE_DIALOG -> {
                    DialogPopup(context, getString(R.styleable.CustomSpinner_android_prompt))
                }
                MODE_BOTTOMSHEET -> {
                    BottomSheetPopup(context, getString(R.styleable.CustomSpinner_android_prompt))
                }
                else -> {
            ...
            getContext().getDrawableCompat(
                getResourceId(
                    R.styleable.CustomSpinner_android_src,
                    getResourceId(
                        R.styleable.CustomSpinner_srcCompat,
    ...
    fun performItemClick(view: View?, position: Int, id: Long): Boolean {
        return run {
            onItemClickListener?.let {
                ...
                it.onItemClick(this@CustomSpinner, view, position, id)
                true
            } ?: false
        }.also {
    ...
    override fun onSaveInstanceState(): Parcelable? {
        return super.onSaveInstanceState()?.let {
            SavedState(it).apply {
                this.selection = this@CustomSpinner.selection
                this.isShowingPopup = this@CustomSpinner.popup.isShowing()
            }
        }
    }
    ...
    private inner class DialogPopup(
        ...
        override fun onClick(dialog: DialogInterface, which: Int) {
            this@CustomSpinner.selection = which
            onItemClickListener?.let {
                this@CustomSpinner.performItemClick(null, which, adapter?.getItemId(which) ?: 0L)
            }
            popup?.dismiss()
        }
    ...
    private inner class DropdownPopup(context: Context, attrs: AttributeSet?) :
        ...
        init {
            ...
            setOnItemClickListener { parent, v, position, id ->
                this@CustomSpinner.selection = position
                onItemClickListener?.let {
                    this@CustomSpinner.performItemClick(
                        v,
                        position,
    ...
    private inner class BottomSheetPopup(
        ...
        override fun show(position: Int) {
            ...
            popup = BottomSheetDialog(context).apply {
                ...
                if (Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN) {
                    textDirection = this@CustomSpinner.textDirection
                    textAlignment = this@CustomSpinner.textAlignment
                }
                setOnDismissListener { listener?.onDismiss() }
            }.also {
                it.show()
    ...
    interface OnItemSelectedListener {
         ...
         */
        fun onItemSelected(parent: CustomSpinner, view: View?, position: Int, id: Long)
         ...
         */
        fun onNothingSelected(parent: CustomSpinner)
    }
    ...
    interface OnItemClickListener {
         ...
         * @param [id] The row id of the item that was clicked.
         */
        fun onItemClick(parent: CustomSpinner, view: View?, position: Int, id: Long)
    }
